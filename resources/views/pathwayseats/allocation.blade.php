 ...
 <select class="form-select" id="semester"  onchange=" return refreshPathwayAllocationList()">
          ...
          @if(isset($semesters))
            @foreach($semesters as $semester)
              <option value="{{$semester->id}}" {{ $activesemester==$semester->id  ?'selected':''}}>{{ $semester->status=="active"  ?'Current: ':($semester->semester_enddt < now()  ?'Prev: ':'')}}{{$semester->semester_desc}}</option>
            @endforeach
          @endif
        </select>
      </div>
